...
/**
 * Script Organization Tool for Tata AI
 * 
 * This script organizes script files into appropriate categories based on their names and content.
 * It creates a directory structure, categorizes scripts, copies them to their proper locations,
 * and generates a report of all moved files.
 */
...
function organizeScripts() {
  ...
  scripts.forEach(scriptPath => {
    // Skip if the script is already in one of our category directories
    if (directories.some(dir => scriptPath.startsWith(dir))) return;
...
function generateReport(moved) {
  ...
  report += `3. Once verified, you can delete the original scripts\n`;
  report += `4. Run \`update_readme.sh\` to generate a comprehensive README for your scripts directory\n`;
